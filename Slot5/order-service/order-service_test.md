...
- [x] (Tùy chọn – nâng cao) Đề xuất thêm validation, viết lại test case kỳ vọng `400 Bad Request`
...
- [x] Gửi lại request, quan sát status code
...
|---|---|---|---|---|
| 1 | Đặt hàng hợp lệ | POST | 201 | [x] |
| 2 | Đặt hàng số lượng lớn, giá thập phân | POST | 201 | [x] |
| 3 | Thiếu `skuCode` (đã có validation) | POST | 400 Bad Request | [x] |
| 4 | Sai kiểu `quantity` | POST | 400 | [x] |
| 5 | Thiếu `Content-Type` / không hợp lệ | POST | 415 | [x] |
| 6 | Body rỗng (đã có validation) | POST | 400 Bad Request | [x] |
| 7 | Sai method (GET) | GET | 405 | [x] |
| 8 | Sai path | POST | 404 | [x] |

- [x] Export Postman Collection ra file `.json` để lưu lại (Collection → `...` → Export)
