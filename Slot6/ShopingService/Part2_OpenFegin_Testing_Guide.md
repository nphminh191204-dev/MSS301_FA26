...
- [x] Phần A – Test thủ công bằng Postman (đủ hàng / hết hàng)
...
- [x] Test thủ công Postman: đơn đủ hàng → `201`
- [x] Test thủ công Postman: đơn vượt tồn kho → `500` + đúng message exception
- [x] Xác nhận đơn hết hàng **không** được lưu vào DB (rollback nhờ `@Transactional`)
- [x] Dependency `spring-cloud-starter-contract-stub-runner` đã thêm
- [x] `InventoryStubs` đã tạo, dùng `urlEqualTo` (hoặc `withQueryParam` nâng cao)
- [x] `src/test/resources/application.properties` trỏ `inventory.url` sang WireMock
- [x] `OrderServiceApplicationTests` viết lại, có gọi `stubInventoryCall` trước khi test
- [x] `mvn test` chạy PASS toàn bộ
- [x] Hiểu được flow test end-to-end (9 bước ở Bước 11)
